...
void Sha1::padding() {
  ...
  int n = m_MessageLen / BYTE;
  int rem = BYTE - m_MessageLen % BYTE;
  int size = (n + 1) * BYTE;

  m_PaddedMessage.resize(size);
  std::fill(m_PaddedMessage.begin(), m_PaddedMessage.end(), 0);
  std::copy(m_Message, m_Message + m_MessageLen, m_PaddedMessage.begin());

  if (rem > 0) {
    m_PaddedMessage[m_MessageLen] |= 0x80;
    m_PaddedMessage[m_PaddedMessage.size() - 1] |= m_MessageLen * 8;
  }
}
